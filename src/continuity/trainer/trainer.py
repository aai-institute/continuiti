...
class Trainer:
    ...
    def fit(
        ...
        Returns:
            Trained operator.
        """
        verbose = (self.device == torch.device("cpu") or self.device == 0)

        # Default callback
        if callbacks is None:
            callbacks = [PrintTrainingLoss()] if verbose else []
        ...
        if dist.is_available() and dist.is_initialized():
            operator = DDP(operator, device_ids=[self.device], output_device=self.device)

...
class DeepONet(Operator):
    ...
    **Note:** This operator is not discretization invariant, i.e., it assumes that all observations were evaluated at
    ...
    def forward(self, _: torch.Tensor, u: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        # dot product
        b = b.reshape(-1, self.dataset_shape.v.num, self.dataset_shape.v.dim, self.basis_functions)
        t = t.reshape(b.size(0), -1, self.dataset_shape.v.num, self.dataset_shape.v.dim, self.basis_functions)
